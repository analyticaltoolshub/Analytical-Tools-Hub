...
for (const entry of pages) {
  test(`${entry.name} loads and its primary sample flow remains operable`, async ({ page }) => {
    ...
    if (entry.name === 'Data Envelopment Analysis') {
      ...
      await expect(page.locator('#scenarioReferenceSelector input:checked')).toHaveCount(3);
      await expect(page.locator('#scenarioReferenceSelector')).toContainText('REFERENCE');
      const scenarioFields = page.locator('#scenarioValueFields input');
      await scenarioFields.nth(0).fill('12');
      await scenarioFields.nth(1).fill('110');
      await scenarioFields.nth(2).fill('120');
      await scenarioFields.nth(3).fill('95');
      await page.locator('#evaluateScenarioButton').click();
      await expect(page.locator('#resourceScenarioResults')).toBeVisible();
      await expect(page.locator('#resourceScenarioInterpretation')).toContainText('scenario was evaluated against 3 selected historical DMUs');
      ...
      await expect(page.locator('#resourceScenarioSummary')).toContainText('Scale efficiency');
